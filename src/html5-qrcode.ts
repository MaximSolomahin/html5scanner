...
export class Html5Qrcode {
        ...
        return new Promise((resolve, reject) => {
            ...
            let renderingCallbacks: RenderingCallbacks = {
                onRenderSurfaceReady: (viewfinderWidth, viewfinderHeight) => {
                    ...
                    this.foreverScanTimeout = setInterval(() => {
                            this.foreverScan(
                                qrCodeSuccessCallback, qrCodeErrorCallbackInternal!
                            );
    ...
    private async foreverScan(qrCodeSuccessCallback: QrcodeSuccessCallback,
        ...
        const sWidthOffset = this.qrRegion.width;
        const sHeightOffset = this.qrRegion.height;
        const sxOffset = (videoElement.videoWidth - this.qrRegion.width) / 2 // this.qrRegion.x * widthRatio;
        const syOffset = (videoElement.videoHeight - this.qrRegion.height) / 2 // this.qrRegion.y * heightRatio;
    ...
    private async mainScan(qrCodeSuccessCallback: QrcodeSuccessCallback,
        ...
        return await this.scanContext(qrCodeSuccessCallback, qrCodeErrorCallback)
            .finally(() => this.scanVariant = 'zoomX2')
    }
    ...
    private async invertScan(
        ...
        return await this.scanContext(qrCodeSuccessCallback, qrCodeErrorCallback)
            .finally(() => this.scanVariant = 'invertZoomX2')
    }
